...
from mat import Mat
from vec import Vec
from matutil import mat2rowdict,mat2coldict,coldict2mat,rowdict2mat
...
def lin_comb_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    return sum([v[k]*Vec(M.D[0],{row:M[row,k] for row in M.D[0]}) for k in v.D])


## Problem 12
def lin_comb_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    return sum([v[k]*Vec(M.D[1],{col:M[k,col] for col in M.D[1]}) for k in v.D])
...
def dot_product_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    u = mat2coldict(M)
    return Vec(M.D[1], {key: v*u[key] for key in M.D[1]})
...
def Mv_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return coldict2mat({col: A*(mat2coldict(B)[col]) for col in B.D[1]})
