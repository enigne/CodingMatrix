...
def vec_select(veclist, k): 
    ...
    True
    '''
    return [veclist[i] for i in range(len(veclist)) if k not in veclist[i].f.keys() or veclist[i][k] == 0]
...
def scale_vecs(vecdict):
    ...
    True
    '''
    return [ (1.0/key)*vecdict[key] for key in vecdict.keys()]
...
def GF2_span(D, L):
    ...
    if (len(L)) <= 1: 
        ...
        sub_span = GF2_span(D,[L[i] for i in range(len(L)-1)])
        linearComb = []
        for k in sub_span:
            linearComb.append(k)
            linearComb.append(k+L[len(L)-1])
        return linearComb
