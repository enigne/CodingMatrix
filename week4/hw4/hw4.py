...
gf2_lc_rep_4 = [0,one,0,0,one,one,0,0]
...
linear_dep_R_2 = [2*math.sqrt(2), math.sqrt(2)/math.pi, 1]
...
indep_vec_1 = Vec({1,2,3}, {1:1})
indep_vec_2 = Vec({1,2,3}, {2:1})
indep_vec_3 = Vec({1,2,3}, {3:1})
indep_vec_4 = Vec({1,2,3}, {1:1,2:1,3:1})
...
def is_superfluous(L, i):
    ...
    A = coldict2mat([L[k] for k in range(len(L)) if k != i])
    u = solve(A,b)
    residual = b - A*u
...
def is_independent(L):
    ...
    for i in range(len(L)):
        if is_superfluous(L,i): return False
    return True
...
def superset_basis(S, L):
    ...
    T = [it for it in L]
    for l in S: 
        if l in L and not is_independent(T):
            T.remove(l)
    return T
