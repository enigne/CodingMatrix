...
def scale(a, b):
    ...
    '''
    return Mat((de_label,de_label),{('x','x'):a, ('y','y'):b, ('u','u'):1})
...
def rotation(angle):
    ...
    '''
    return Mat((de_label,de_label), {('x','x'):math.cos(angle), ('x','y'):-math.sin(angle), ('y','x'):math.sin(angle),  ('y','y'):math.cos(angle), ('u','u'):1})
...
def reflect_y():
    ...
    '''
    return Mat((de_label,de_label),{('x','x'):-1, ('y','y'):1, ('u','u'):1})
...
def grayscale():
    ...
    '''
    return Mat(({'r','g','b'},{1}),{('r',1):1,('g',1):1,('b',1):1})*Mat(({1},{'r','g','b'}),{(1,'r'):77/256,(1,'g'):151/256,(1,'b'):28/256})
...
def reflect_about(p1,p2):
    ...
    '''
    angle = math.atan((p1[0]-p2[0])/(p1[1]-p2[1])) if p1[1] != p2[1] else math.pi/2
    return translation(p1[0],p1[1])*rotation(-angle)*reflect_y()*rotation(angle)*translation(-p1[0],-p1[1])
